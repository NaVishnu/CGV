...
static vector<vector<float>> multiply(vector<vector<float>> a, vector<vector<float>> b) {
    int rows = a[0].size();
    int columns = b.size();
    int cols = a.size();

    vector<vector<float>> c(rows, vector<float> (columns, 0));
...
static void display() {
    ...
    for (int i=0; i<cnt-2; i++) {
        Point p4 = transforms[i];
        Point p5 = transforms[i+1];
        Point p6 = transforms[i+2];
