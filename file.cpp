...
static vector<vector<double>> multiply(vector<vector<double>> a, vector<vector<double>> b) {
    ...
    vector<vector<double>> c(rows, vector<double> (columns, 0));
...
static void transformations(int key_, int xx, int yy) {
    ...
    const int altx = (tx1+tx2+tx3)/3;
    const int alty = (ty1+ty2+ty3)/3;
    vector<vector<double>> origintransformMatrix {
        {1, 0, -1 * altx},
        {0, 1, -1 * alty},
        {0, 0, 1}
    };
    ...
    vector<vector<double>>scaleTransformMatrix {
        {0.5,0,0},
        {0,0.5,0},
        {0,0,1},
    };
    vector<vector<double>>bigscaleTransformMatrix {
        {2,0,0},
        {0,2,0},
        {0,0,1},
    };
    int theta = 1;
    vector<vector<double>>rotateTransformation {
        {cos(theta),-1 * sin(theta),0},
        {sin(theta),cos(theta),0},
        {0,0,1},
    };
    vector<vector<double>>shearTransformation {
        {1, 2, 0},
        {2, 1, 0},
        {0,0,1},
    };
    ...
    switch(key_) {
        case GLUT_KEY_UP:
            ...
            result = multiply(origintransformMatrix, object);
            result = multiply(scaleTransformMatrix, result);
            result = multiply(transformMatrix, result);
        ...
        case GLUT_KEY_DOWN:
            result = multiply(origintransformMatrix, object);
            result = multiply(bigscaleTransformMatrix, result);
            result = multiply(transformMatrix, result);
            tx1 = result[0][0];
            tx2 = result[0][1];
            tx3 = result[0][2];
        ...
        case GLUT_KEY_LEFT:
            result = multiply(origintransformMatrix, object);
            result = multiply(rotateTransformation, result);
            result = multiply(transformMatrix, result);
        ...
        case GLUT_KEY_RIGHT:
            result = multiply(origintransformMatrix, object);
            result = multiply(shearTransformation, result);
            result = multiply(transformMatrix, result);
